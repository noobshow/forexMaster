...
{
public:
  ServerWrapper( std::set<sys_socket_t> sockets, SocketServer& server,
...
private:
  ...
  void onEvent( SocketMonitor& monitor, sys_socket_t socket )
  {
  ...
  void onError( SocketMonitor& monitor, sys_socket_t socket )
  {
...
sys_socket_t SocketServer::add( int port, bool reuse, bool noDelay,
                       int sendBufSize, int rcvBufSize )
THROW_DECL( SocketException& )
{
  if( m_portToInfo.find(port) != m_portToInfo.end() )
    return m_portToInfo[port].m_socket;

  sys_socket_t socket = socket_createAcceptor( port, reuse );
  if( socket < 0 )
...
sys_socket_t SocketServer::accept(sys_socket_t socket )
{
  SocketInfo info = m_socketToInfo[socket];

  sys_socket_t result = socket_accept( socket );
  if( info.m_noDelay )
...
int SocketServer::socketToPort(sys_socket_t socket )
{
...
sys_socket_t SocketServer::portToSocket( int port )
{
