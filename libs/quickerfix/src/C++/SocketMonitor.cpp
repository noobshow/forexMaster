...
: m_timeout( timeout )
{
  ...
  std::pair<sys_socket_t, sys_socket_t> sockets = socket_createpair();
...
bool SocketMonitor::addConnect(sys_socket_t s )
{
...
bool SocketMonitor::addRead(sys_socket_t s )
{
  socket_setnonblock( s );
...
bool SocketMonitor::addWrite(sys_socket_t s )
{
...
bool SocketMonitor::drop(sys_socket_t s )
{
  Sockets::iterator i = m_readSockets.find( s );
...
double elapsed = ( double ) ( clock() - m_ticks ) / ( double ) CLOCKS_PER_SEC;
  if ( elapsed >= timeout || elapsed == 0.0 )
  {
    m_ticks = clock();
    m_timeval.tv_sec = 0;
    m_timeval.tv_usec = static_cast<long>(timeout * 1000000);
  }
  else
  {
    m_timeval.tv_sec = 0;
    m_timeval.tv_usec = static_cast<long>( ( timeout - elapsed ) * 1000000 );
  }
  return &m_timeval;
#endif
}
...
void SocketMonitor::signal(sys_socket_t socket )
{
  socket_send( m_signal, (char*)&socket, sizeof(socket) );
}

void SocketMonitor::unsignal(sys_socket_t s )
{
  Sockets::iterator i = m_writeSockets.find( s );
...
void SocketMonitor::processReadSet( Strategy& strategy, fd_set& readSet )
{
  ...
  {
	sys_socket_t s = readSet.fd_array[ i ];
    if( s == m_interrupt )
    {
...
void SocketMonitor::processWriteSet( Strategy& strategy, fd_set& writeSet )
{
  ...
  {
	sys_socket_t s = writeSet.fd_array[ i ];
...
void SocketMonitor::processExceptSet( Strategy& strategy, fd_set& exceptSet )
{
  ...
  {
	sys_socket_t s = exceptSet.fd_array[ i ];
